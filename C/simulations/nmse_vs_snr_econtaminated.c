 ...
 *
*  @details Figure 2(b) shows the curves of the normalized mean square error (NMSE) in dB of the recovered coefficient vector versus the signal-to-noise ratio (SNR) in dB of the random projections. For this experiment, the additive perturbations are characterized using an \f$ \varepsilon \f$-contaminated noise model (also known as p-Gaussian noise) with impulsiveness level of \f$ \varepsilon = 3\% \f$ and \f$ \sigma_2^2 = 100 \sigma_1^2 \f$. More precisely, the variance of the Gaussian noise \f$\sigma_1^2\f$ is computed from the SNR values under evaluation. Furthermore, this experiment obtains the error data generated by both the proposed sparse signal recovery algorithm (WMR-AR, weighted median regressions with adaptive regularizer) and the method based on weighted median regressions with hard thresholding (WMR-HT). Each point in the curves are obtained by averaging a user-defined number of trials, which is introduced in running time. The results of the simulation are saved in the results directory and the curves can be visualized by running the @c Figure2b.m routine. This plotting routine can be executed under either Octave or Matlab.
 *
...
int main()
{
	...
	*/
	...
	printf("\t\tWMR-AR\t\tWMR-HT\t\t(sec)\t\ttime (min) \n");
	printf("*************************************************************************\n");
